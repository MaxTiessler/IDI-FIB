...
void MyGLWidget::valorsInicials()
{
  ...
  centreEngraA    = glm::vec3(-0.3, 0.2, 0);  // centreA
  centreEngraB    = glm::vec3(0.4, -0.2, 0);  // centreB
  centrePinyo     = glm::vec3(0, -0.2, 0);    // centreP
  rPinyo          = .15;                      // radiP
  rEngraA         = .35;                      // radiA
  rEngraB         = .25;                      // radiB
  anglePinyo      = 0;                        // angle de gir Pinyo
  angleEngraA     = 0;                        // angle de gir A
  angleEngraB     = 0;                        // angle de gir B
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    case Qt::Key_M:
      if(!alternate)
      {
        //aprofito que deltaTriangle és 6 com els graus sexagesimals que ha de rotar
        anglePinyo += deltaTriangle;
        angleEngraB -= relacioPEB*deltaTriangle;
        angleEngraA -= relacioPEA*deltaTriangle;
      }
      else
      {
          anglePinyo -= deltaTriangle;
